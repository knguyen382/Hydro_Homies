...
void waterIntakeCalculation(void * pvParameters)
{
  ...
  while (1)
  {
    ...
    double water_intake_remaining = 2000;
    double total_water_intake_today = 0;
    ...
    if (water_intake_times == 0)
    {
        ...
        drink_time = get_time('m');

    }
    // consumed water
    if((current_volume < temp_volume) && (get_time('m') - drink_time >= 5) && (get_time('h') - last_water_consumed_time = 0))
    {
        //water_consumed[water_intake_times - 1] = current_volume - temp_volume;
        //water_intake_times++;
        total_water_intake_today = total_water_intake_today + (temp_volume - current_volume);
        water_intake_remaining = water_intake_remaining - total_water_intake_today;
        temp_volume = current_volume;
        last_water_consumed_time = get_time('h');
        drink_time = get_time('m');
    }
    if((current_volume < temp_volume) && get_time('m') - drink_time < 0 && (get_time('h') - last_water_consumed_time = 1))
    {
        total_water_intake_today = total_water_intake_today + (temp_volume - current_volume);
        water_intake_remaining = water_intake_remaining - total_water_intake_today;
        temp_volume = current_volume;
    ...
    // water is refilled
    else if ((current_volume > temp_volume) && (get_time('m') - drink_time >= 5) && (get_time('h') - last_water_consumed_time = 0))
    {
        temp_volume = current_volume;
    }
    else if ((current_volume < temp_volume) && get_time('m') - drink_time < 0 && (get_time('h') - last_water_consumed_time = 1))
    {
        temp_volume = current_volume;
    }
    // no water is consumed or refill for 2 hour
    else if ((current_volume == temp_volume) && (get_time('h') - last_water_consumed_time >= 2))
    {
        alarm();
    }
    if(get_time('h') == 0 && get_time('m') == 0)
    {
        water_intake_remaining = 2000;
...
double total_water_intake_in_day()
{
